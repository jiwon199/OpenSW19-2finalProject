...
url = 'https://kr.socialerus.com/ranking/index.html?sort=&rank=&term=&cate=13' #소셜러스_동물/펫
...
try:
    for i in range(1,11):
        print(names[i-1].text, subscribers[i-1].text, views[i-1].text, "동물/펫")
        cur.execute("INSERT INTO channels VALUES (%s,%s,%s,%s,%s)", (i+40, names[i-1].text , subscribers[i-1].text , views[i-1].text , "동물/펫"))
        conn.commit() 
