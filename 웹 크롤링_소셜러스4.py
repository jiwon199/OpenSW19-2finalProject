...
url = 'https://kr.socialerus.com/ranking/index.html?sort=&rank=&term=&cate=4' #소셜러스_엔터테인먼트
...
try:
    for i in range(1,11):
        print(names[i-1].text, subscribers[i-1].text, views[i-1].text, "음악/댄스")
        cur.execute("INSERT INTO channels VALUES (%s,%s,%s,%s,%s)", (i+30, names[i-1].text , subscribers[i-1].text , views[i-1].text , "음악/댄스"))
        conn.commit() 
