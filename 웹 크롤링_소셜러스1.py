...
url = 'https://kr.socialerus.com/ranking/index.html?sort=&rank=&term=&cate=6' #소셜러스_엔터테인먼트
...
try:
    for i in range(1,11):
        print(names[i-1].text, subscribers[i-1].text, views[i-1].text, "엔터테인먼트")
        cur.execute("INSERT INTO channels VALUES (%s,%s,%s,%s,%s)", (i, names[i-1].text , subscribers[i-1].text , views[i-1].text , "엔터테인먼트"))
        conn.commit() 
